...
class Billing(View):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            NewUserModel.objects.filter(username=request.user).update(wallet= int(request.POST.get('wallet'))+int(walletamount))
            ...
            if couponForm.is_valid():
                ...
                if walletamount >= couponamount:
                    ...
                    if couponForm.username == request.user.username:                        
                        couponForm.save()
